...
// Gabarit distinct des autres — ton "on a réservé pour vous, il ne reste
// qu'à payer", pas une confirmation ni une mauvaise nouvelle. Le lien est
// la seule action possible pour ce destinataire : il n'a pas de mot de
// passe à connaître, jamais mentionné ici.
function renderBookingPaymentLinkEmailHtml(payload: BookingPaymentLinkPayload): string {
  return `
    ...
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:24px;">
        <p style="margin:0 0 4px;color:${TEXT};font-size:20px;font-weight:800;">
          Une réservation vous attend
        </p>
        <p style="margin:0 0 20px;color:${MUTED};font-size:14px;">
          ${payload.companyName} a préparé cette réservation pour vous. Il ne reste qu'à la payer
          en toute sécurité via le lien ci-dessous — aucun compte ni mot de passe n'est nécessaire.
        </p>
        ...
        <div style="border-radius:12px;background:${PRIMARY}1a;padding:16px 20px;margin-bottom:20px;">
          ...
          <p style="margin:0;color:${TEXT};font-size:13px;">
            Ce lien expire le ${formatDateTime(payload.expiresAt)} — passé ce délai, contactez
            ${payload.companyName} pour une nouvelle réservation.
          </p>
        </div>

        <div style="text-align:center;">
          <a href="${payload.paymentUrl}" style="display:inline-block;background:${PRIMARY};color:#1a1a1a;text-decoration:none;font-weight:700;font-size:14px;padding:12px 24px;border-radius:10px;">
            Payer maintenant
          </a>
        </div>
      </div>
...
export async function sendBookingPaymentLinkEmail(payload: BookingPaymentLinkPayload): Promise<void> {
  ...
  if (error) {
    throw new Error(`Resend a refusé l'envoi (lien de paiement) : ${error.message}`);
  }
}
