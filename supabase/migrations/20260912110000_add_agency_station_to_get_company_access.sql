-- Chantier : sélecteur de gare dans la topbar du back-office
--
-- get_company_access() gagne agency_station_id, pour que requireCompany()
-- (dal.ts) connaisse la gare de rattachement de l'agent — c'est le défaut
-- du sélecteur de gare. agencies est déjà left-joinée (alias ag) : on
-- ajoute seulement ag.station_id, aucune jointure supplémentaire.
-- agencies.station_id est not null, donc la valeur est toujours présente
-- quand l'agent a une agence ; elle est null pour un propriétaire (qui
-- n'a pas d'agence — check company_members_agency_matches_role).
--
-- Signature de retour modifiée : drop + create (jamais create or replace
-- sur un changement de colonnes de returns table), puis re-grant.
